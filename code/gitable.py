...
def dump1(u,issues):
  ...
  for event in w:
    ...
    eventObj1.append(created_at)
    eventObj1.append(action)
    eventObj1.append(label_name)
    eventObj1.append(username)
    eventObj1.append(milestone)
	
    all_events = issues.get(issue_id)
    if not all_events: all_events = []
    all_events.append(eventObj1)
...
def launchDump():
  ...
  while(True):
    ...
    page += 1
    if not doNext : break
  with open("events_output_team6.csv", 'w') as file:
	file.writelines("Issue" + "," + "Timestamp" + "," + "Action" + "," + "Label" + "," + "User" + "," + "Milestone" + '\n')
